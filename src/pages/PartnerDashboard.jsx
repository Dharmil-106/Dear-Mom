...
const PartnerDashboard = () => {
  ...
  const name = localStorage.getItem('dearMomName') || 'Partner';
  const { currentWeek, weeksLeft, trimester, daysLeft } = getDueDateInfo();
      ...
      <div className="max-w-2xl mx-auto px-4 py-6 space-y-4">
        ...
        <div className="bg-gradient-to-br from-amber-50 to-yellow-50 rounded-2xl p-5 border border-amber-100 card-hover animate-fade-in" style={{ animationDelay: '0.1s' }}>
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="font-semibold text-gray-800 text-sm mb-1">Support Tip of the Day</h3>
              <p className="text-gray-600 text-sm leading-relaxed">{todayTip}</p>
            </div>
          </div>
        </div>

        {/* Her Mood + Baby Size row */}
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="bg-white rounded-2xl p-5 shadow-sm border border-rose-50 card-hover animate-fade-in" style={{ animationDelay: '0.2s' }}>
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="font-semibold text-gray-800 text-sm">Her Mood</h3>
            </div>
            {latestMood ? (
              <>
                <p className="text-3xl mb-1">{latestMood.emoji}</p>
                <p className="text-sm text-gray-600">{latestMood.label}</p>
                <p className="text-xs text-gray-400 mt-1">{getMoodSuggestion(latestMood)}</p>
              </>
            ) : (
              <p className="text-sm text-gray-400">No mood logged yet</p>
            )}
          </div>
          ...
          <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-2xl p-5 border border-green-100 card-hover animate-fade-in" style={{ animationDelay: '0.3s' }}>
            <div className="flex items-center gap-2 mb-3">
              <Baby size={18} className="text-green-500" />
              <h3 className="font-semibold text-gray-800 text-sm">Baby</h3>
            </div>
        ...
        <div className="animate-fade-in" style={{ animationDelay: '0.5s' }}>
          ...
          <div className="grid grid-cols-2 gap-3">
            <button
              onClick={() => navigate('/partner/games')}
              className="flex items-center gap-3 bg-orange-50 rounded-2xl p-4 hover:scale-[1.02] transition-transform"
            >
              <div className="bg-orange-100 p-2.5 rounded-xl">
                <Gamepad2 size={20} className="text-orange-500" />
              </div>
              <div className="text-left">
                <p className="font-semibold text-sm text-gray-800">Games 🎮</p>
                <p className="text-xs text-gray-400">Fun activities</p>
              </div>
            </button>
            <button
              onClick={() => navigate('/partner/memory')}
              className="flex items-center gap-3 bg-pink-50 rounded-2xl p-4 hover:scale-[1.02] transition-transform"
            >
              <div className="bg-pink-100 p-2.5 rounded-xl">
                <BookHeart size={20} className="text-pink-500" />
              </div>
              <div className="text-left">
                <p className="font-semibold text-sm text-gray-800">Memory 📖</p>
                <p className="text-xs text-gray-400">Precious moments</p>
              </div>
            </button>
          </div>
        </div>
