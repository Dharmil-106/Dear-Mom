...
const MoodTracker = () => {
  ...
  useEffect(() => {
    const storedMoods = JSON.parse(localStorage.getItem('dearMomMoods') || '[]');
    setMoods(storedMoods);

    // Check if today's mood already exists
    const todayMood = storedMoods.find((m) => m.date === todayKey);
    if (todayMood) {
  ...
  const handleSave = () => {
    ...
    const existingIndex = updatedMoods.findIndex((m) => m.date === todayKey);
    if (existingIndex >= 0) {
      updatedMoods[existingIndex] = entry;
    } else {
      updatedMoods.push(entry);
    }

    localStorage.setItem('dearMomMoods', JSON.stringify(updatedMoods));
  ...
  const getStreak = () => {
    ...
    for (let i = 0; i <= 365; i++) {
      ...
      date.setDate(date.getDate() - i);
      const key = date.toISOString().split('T')[0];
      const hasMood = moods.some((m) => m.date === key);
      if (hasMood) {
        streak++;
      } else {
        break;
              ...
              return (
                <button
                  ...
                  className={`flex flex-col items-center gap-1 p-3 rounded-2xl border-2 transition-all duration-200 ${
                    isSelected
                      ? `${mood.activeColor} shadow-md scale-110`
        ...
        <div className="animate-fade-in" style={{ animationDelay: '0.2s' }}>
          ...
          <div className="flex gap-2 overflow-x-auto pb-2">
            {last7.map((day) => (
              <div
                key={day.key}
                className={`flex-shrink-0 flex flex-col items-center gap-1.5 rounded-xl p-3 min-w-[60px] ${
                  day.key === todayKey
